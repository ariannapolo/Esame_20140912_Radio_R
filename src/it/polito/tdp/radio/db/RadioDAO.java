...
public class RadioDAO {
	...
	public List<Ponte> getPontiInComune(Citta c1, Citta c2, Map<Integer,Ponte> ponti){
		final String sql = "SELECT c1.idPonte FROM copertura c1 , copertura c2 WHERE c1.idCitta = ? AND c2.idCitta = ? AND c1.idPonte = c2.idPonte";

		List<Ponte> pontiC = new ArrayList<Ponte>();
		...
		try {
			...
			while (rs.next()) {
				Ponte p = ponti.get(rs.getInt("idPonte"));
				pontiC.add(p);
			}
			return pontiC;
		} catch (SQLException e) {
	...
	public void setPonti(Citta c, Map<Integer,Ponte> ponti){
		final String sql = "SELECT idPonte FROM copertura WHERE idCitta = ?";
		List<Ponte> pontiC = new ArrayList<Ponte>();
